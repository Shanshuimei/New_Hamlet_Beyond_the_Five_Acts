...
interface AppConfig {
  ...
  borderRadius?: number;
  font?: string;
  onItemSelect?: (index: number) => void;  // 添加回调函数
}
...
class App {
  ...
  constructor(
    ...
    this.onItemSelect = onItemSelect;  // 保存回调函数
    this.createRenderer();
    this.createCamera();
    this.createScene();
    this.onResize();
  ...
  // 修改 onTouchDown 方法，添加点击检测
  onTouchDown(e: MouseEvent | TouchEvent) {
    this.isDown = true;
    this.scroll.position = this.scroll.current;
    this.start = "touches" in e ? e.touches[0].clientX : e.clientX;
    
    // 记录点击开始位置和时间，用于区分点击和拖动
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    // console.log(`CircularGallery: 滚动目标更新为 ${this.scroll.target}`);
  }

  // 修改 onTouchUp 方法，添加点击检测
  onTouchUp(e: MouseEvent | TouchEvent) {
    this.isDown = false;
    
    // 检测是否为点击事件（移动距离小且时间短）
    const clickEndX = "changedTouches" in e ? e.changedTouches[0].clientX : e.clientX;
    const clickEndTime = Date.now();
    const moveDistance = Math.abs(clickEndX - this.clickStartX);
    const clickDuration = clickEndTime - this.clickStartTime;
    
    if (moveDistance < 10 && clickDuration < 300) {
      // 这是一个点击事件，找出点击的是哪个项目
      this.handleItemClick(clickEndX);
    } else {
      // 这是一个拖动事件，执行原有的检查逻辑
      this.onCheck();
    }
  ...
  // 添加新方法，处理项目点击
  handleItemClick(clientX: number) {
    ...
    let minDistance = Math.abs(this.medias[0].plane.position.x - normalizedX * this.viewport.width / 2);
    
    for (let i = 1; i < this.medias.length; i++) {
      const distance = Math.abs(this.medias[i].plane.position.x - normalizedX * this.viewport.width / 2);
    ...
    // 计算选中项的索引（考虑到重复项）
    const selectedIndex = this.medias.indexOf(closestMedia) % (this.mediasImages.length / 2);
    
    // 如果提供了回调函数，则调用它
    if (this.onItemSelect) {
      console.log(`CircularGallery: 点击选择了索引 ${selectedIndex}`);
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, onItemSelect]);  // 添加依赖项
